  ...
  <div class="global-view space-y-12">
    ...
    <section class="space-y-6">
      <div class="p-6 bg-white rounded-2xl border border-sky-100 shadow-sm">
        ...
        <p class="text-sm text-slate-600 mb-6">
          適用場景：<strong>兄弟組件</strong>、<strong>不相關組件</strong>。透過 <code>mitt</code> 或 <code>bus.ts</code> 發送與監聽事件，不需建立層級關係。
        </p>
    ...
    <section class="space-y-6">
      <div class="p-6 bg-white rounded-2xl border border-rose-100 shadow-sm">
        ...
        <p class="text-sm text-slate-600 mb-6">
          適用場景：<strong>祖父傳孫子</strong>。中間組件不需宣告 Props，只需透傳 <code>v-bind="$attrs"</code>，孫子即可接收所有屬性與事件。
        </p>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-10">
          <div class="space-y-4">
            ...
            <div class="p-6 bg-slate-50 rounded-2xl border-2 border-slate-200 shadow-inner">
              <input v-model="ancestorMsg" type="text" class="w-full px-4 py-2 border rounded-lg mb-4">
              <div class="flex items-center justify-between">
                <span>祖父計數: <strong class="text-xl text-rose-500">{{ ancestorCount }}</strong></span>
                <button @click="ancestorCount++" class="px-4 py-2 bg-rose-500 text-white rounded-lg">+1</button>
              </div>
            </div>
          </div>
